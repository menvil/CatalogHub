<?php

namespace App\Actions\CentralCatalog;

use App\Enums\AuditAction;
use App\Enums\AuditContext;
use App\Models\CentralCatalog\CentralBrand;
use App\Models\MediaAsset;
use App\Models\MediaAssignment;
use App\Models\User;
use App\Services\Audit\AuditRecorder;
use Illuminate\Support\Facades\DB;

final readonly class SetCentralBrandLogoAction
{
    public function __construct(private AuditRecorder $audit) {}

    public function execute(User $actor, CentralBrand $brand, MediaAsset $asset): MediaAssignment
    {
        return DB::transaction(function () use ($actor, $brand, $asset): MediaAssignment {
            CentralBrand::query()->lockForUpdate()->findOrFail($brand->id);
            $assignment = MediaAssignment::query()->forEntity(MediaAssignment::ENTITY_TYPE_CENTRAL_BRAND, $brand->id)->forRole(MediaAssignment::ROLE_BRAND_LOGO)->whereNull('locale')->whereNull('site_id')->whereNull('market_id')->first();
            if ($assignment instanceof MediaAssignment) {
                $oldAssetId = (int) $assignment->media_asset_id;

                if ($oldAssetId === (int) $asset->id) {
                    return $assignment;
                }

                $assignment->update(['media_asset_id' => $asset->id, 'position' => 0, 'is_primary' => true, 'visibility' => 'global']);
                $this->audit->record(AuditAction::CatalogBrandLogoAssigned, AuditContext::Central, $actor, $brand, null, ['media_asset_id' => $oldAssetId], ['media_asset_id' => (int) $asset->id]);

                return $assignment;
            }

            $assignment = MediaAssignment::query()->create(['media_asset_id' => $asset->id, 'entity_type' => MediaAssignment::ENTITY_TYPE_CENTRAL_BRAND, 'entity_id' => $brand->id, 'role' => MediaAssignment::ROLE_BRAND_LOGO, 'position' => 0, 'locale' => null, 'site_id' => null, 'market_id' => null, 'is_primary' => true, 'visibility' => 'global']);
            $this->audit->record(AuditAction::CatalogBrandLogoAssigned, AuditContext::Central, $actor, $brand, null, ['media_asset_id' => null], ['media_asset_id' => (int) $asset->id]);

            return $assignment;
        });
    }
}

...
final readonly class SaveBrandTranslationAction
{
    ...
    public function handle(User $actor, CentralBrand $brand, Locale $locale, BrandTranslationInput $input): BrandTranslation
    {
        $translation = DB::transaction(function () use ($actor, $brand, $locale, $input): BrandTranslation {
            ...
            $existing = BrandTranslation::query()->where('brand_id', $lockedBrand->id)->where('locale_id', $locale->id)->first();
            $values = [
                'locale' => $locale->code,
                'name' => $input->name,
                ...
                'source_hash' => $this->hashService->forBrand($lockedBrand),
            ];
            $beforeValues = null;
            ...
            BrandTranslation::query()->upsert([['brand_id' => $lockedBrand->id, 'locale_id' => $locale->id, ...$values]], ['brand_id', 'locale_id'], array_keys($values));
            $saved = BrandTranslation::query()->where('brand_id', $lockedBrand->id)->where('locale_id', $locale->id)->firstOrFail();

...
{
    public function __invoke(CentralBrand $brand, CentralBrandDetailQuery $query, CentralBrandMediaQuery $media, MediaUrlGenerator $urls): View
    {
        return view('central-admin.brands.show', [
            'brand' => $query->loadUsage($brand),
            'logoAsset' => $media->logoFor($brand),
            'mediaUrls' => $urls,
        ]);
    }
}

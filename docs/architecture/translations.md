...
Each Brand/locale has at most one row. The row stores both the Locale foreign key and locale code using the established translation schema, uses the shared status enum, and carries the common `source_hash`, `approved_at`, and `approved_by_user_id` metadata. Physical Brand or Locale deletion cascades to prevent orphan translations; deleting an approver only nulls the approval actor.

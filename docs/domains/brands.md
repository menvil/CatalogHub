...
`CentralBrand` and `Organization` are separate canonical concepts. A Brand is a commercial/catalog identity; an Organization is the legal or corporate entity that may directly own it. Phase 16 persists Organization identity in `organizations` with `id`, display `name`, full case-folded `normalized_name`, a bounded indexed `normalized_name_prefix`, and timestamps. The full value is text so Unicode case-fold expansion cannot overflow a valid 255-character name; the portable 191-character prefix is the search index key. The internal database ID is authoritative. Name search is bounded to 20, options display their IDs, and exact `#ID` lookup keeps identical-name Organizations distinguishable and reachable; there is no automatic merge or global name uniqueness.

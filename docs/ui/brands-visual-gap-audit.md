...
| Rich search/filter/action composition | Name/slug/company search; consistent Country, coverage, translation and quality filters; right-aligned global Clear; overflow actions | approved read model and routes | Converged (D) | Phase 18.1 | Explicit 6/3/2/1 control grids prevent intermediate-width overflow; query state persists through sort/page/per-page and history. Footer selects and row actions use viewport-aware placement. |

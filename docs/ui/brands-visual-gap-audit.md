...
CA-014 A/B/D work is closed and final-converged. Before convergence, a tall preview, permanently open uploader, full-width empty Variants card, always-rendered six-card media selector, and prominent red removal control made the page read as a technical file warehouse. The final `brand-media-v4` composition unifies a 256px contained canonical logo and its actions with a same-height compact Asset details rail inside one approximately 70/30 workspace. Replace is primary, Choose remains secondary, and `Remove logo from brand` lives in the established overflow. Generated variants is a separate compact section below the workspace, so an empty state no longer creates a tall rail. Empty, Processing, Failed, and Unavailable are explicit delivery states with the same replacement recovery actions.
...
The reviewed 1440×1000 final frame shows the complete Primary logo workspace, all Asset details, and Generated variants without scrolling; 1024 retains the aligned two-column workspace, while 768×1024 and 390×844 preserve the stacked mobile order and have no horizontal overflow. The separate 1440×1000 picker frame verifies a compact four-column card grid. Dark/light logos, wordmarks as a separate role, favicon, hero, OG, localized or Site media, Media Completeness, alt-text completeness, Edit Image, and Delete Asset remain intentional architectural divergences. CA-014 manages one exact global primary `brand_logo` assignment and never deletes the Shared Media asset.

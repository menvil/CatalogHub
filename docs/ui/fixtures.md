...
- `BrandFormFixture` provides `brand-form-v2`: one fixed Draft Brand (`Samsung Form Fixture`, ID `13013`) with canonical slug, website, Country reference, and timestamps for CA-013 edit/browser/visual coverage. The browser harness loads it separately after `BrandListFixture`; it is never production demo data.
- `BrandDetailFixture` provides `brand-detail-v2`: the fixed active Samsung list record with a Country reference and three explicit canonical Product references plus the existing archived Sony and Draft Zotac list records for state/lifecycle coverage. It creates no extra Brands, uses no Faker, and is loaded only by the browser/visual harness after the list/form fixtures; it is never production demo data.

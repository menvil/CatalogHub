...
- `BrandDetailFixture` provides `brand-detail-v1`: the fixed active Samsung list record with three explicit canonical Product references plus the existing archived Sony and Draft Zotac list records for state/lifecycle coverage. It creates no extra Brands, uses no Faker, and is loaded only by the browser/visual harness after the list/form fixtures; it is never production demo data.

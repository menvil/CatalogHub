...
Brand summary is the single snapshot surface. It shows current non-archived Product count and distinct derived Category count once; Translation coverage is presented once as part of the merged Quality/Translation health context. The removed Product Portfolio contributed no distinct information. Classification is integrated below the identity columns and exclusively owns grouped derived Category chips/counts and editorial Tag chips, so taxonomy is not repeated and no Brand↔Category mutation is implied.

...
CA-014 Phase 6 v1 manages one global primary Brand logo. Its states are: `no-logo` (upload invitation), `logo-ready` (resolved semantic preview and master metadata), `logo-processing` (the master remains a non-blocking fallback while a requested variant is pending), and `upload-error` (field-level safe validation error). A Brand manager can upload or replace a JPEG, PNG or WebP logo, or detach it with confirmation. Removal retains the asset in Media Library. The preview prefers a ready `brand_logo_512` variant, then 256/128, then the normalized master; failed, processing, or missing files are never selected. If neither a ready variant nor the master file is readable, the view reports the assigned logo as unavailable instead of rendering a broken image. The responsive preview uses `object-contain` on a neutral checkerboard background and metadata wraps on mobile.

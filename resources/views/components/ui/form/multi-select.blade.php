...
<x-ui.form.field :id="$id" :label="$label" :required="$required" :help="$help" :error="$error">
    <select id="{{ $id }}" name="{{ str_ends_with($name, '[]') ? $name : $name.'[]' }}" multiple size="{{ $size }}" @required($required) @disabled($disabled)
        ...
        {{ $controlAttributes->class('block w-full cursor-pointer rounded-admin-input border border-admin-border bg-admin-surface p-1 text-sm text-admin-text focus:border-admin-primary focus:outline-none focus:ring-2 focus:ring-admin-primary/20 disabled:cursor-not-allowed disabled:opacity-60') }} data-ui-multi-select>

...
<x-ui.form.field :id="$id" :label="$label" :required="$required" :help="$help" :error="$error">
    <select id="{{ $id }}" name="{{ $name }}" @required($required) @disabled($disabled)
        ...
        {{ $controlAttributes->class('block min-h-10 w-full rounded-admin-input border border-admin-border bg-admin-surface px-3 py-2 text-sm text-admin-text focus:border-admin-primary focus:outline-none focus:ring-2 focus:ring-admin-primary/20 disabled:opacity-60') }}>

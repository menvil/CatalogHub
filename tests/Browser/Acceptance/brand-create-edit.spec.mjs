...
const formFixtureId = 3
const ownershipFixtureId = 13016

async function chooseParentCompanyAction(ownership, name) {
    await ownership.locator('summary[aria-label="More Parent Company actions"]').click()
    await ownership.getByRole('menuitem', { name, exact: true }).click()
}
...
test('CA-013 creates a normalized draft and returns to Brands', async ({ page }) => {
    ...
    await page.getByRole('link', { name: 'New Brand', exact: true }).click()
    ...
    const createdRow = page.locator('tr[data-row-id]').filter({
        has: page.getByText('Samsung Electronics', { exact: true }),
    })
    await createdRow.locator('summary[aria-label^="Open actions for row"]').click()
    await createdRow.getByRole('menuitem', { name: 'View', exact: true }).click()
    ...
    await expect(page.locator('[data-screen-region="online-presence"]').getByRole('link', { name: 'Open support site', exact: true })).toHaveAttribute('href', 'https://www.samsung.com/support/')
...
test('CA-013 supports keyboard Country selection, Escape, change, and clear', async ({ page }) => {
    ...
    const country = page.getByRole('combobox', { name: 'Country' })
    const countryToggle = page.getByRole('button', { name: 'Open Country options', exact: true })
    await expect(countryToggle).toHaveCSS('cursor', 'pointer')
    await countryToggle.click()
    await expect(page.locator('#brand-country-listbox')).toBeVisible()
...
test('CA-013 displays server validation, retains submitted input, and writes nothing partially', async ({ page }) => {
    ...
    await page.reload()
    await expect(page.locator('#brand-name')).toHaveValue('Apple')
    await expect(page.getByLabel('Website')).toHaveValue('https://www.apple.com/')
...
test('CA-013 edits canonical fields while preserving slug and lifecycle', async ({ page }) => {
    ...
    await signIn(page, 'central', foundationDemo.centralAdmin)
    await expect(page.locator('[data-screen-id="CA-001"]')).toBeVisible()
    await page.goto('/admin/central/brands?q=Apple')
    const fixtureRow = page.locator(`[data-row-id="${formFixtureId}"]`)
    await fixtureRow.locator('summary[aria-label^="Open actions for row"]').click()
    await fixtureRow.getByRole('menuitem', { name: 'Edit', exact: true }).click()
    await expect(page).toHaveURL(new RegExp(`/admin/central/brands/${formFixtureId}/edit$`))
    ...
    await expect(page.getByLabel('Slug')).toHaveValue('apple')
...
test('CA-013 create and edit remain usable without horizontal overflow at 390px', async ({ page }) => {
    ...
    for (const url of ['/admin/central/brands/create', `/admin/central/brands/${formFixtureId}/edit`]) {
        ...
        const secondaryAction = url.endsWith('/create') ? 'Cancel' : 'Back to Overview'
        await expect(page.getByRole('link', { name: secondaryAction, exact: true })).toBeVisible()
        await expect(page.getByRole('button', { name: url.endsWith('/create') ? 'Create Brand' : 'Save changes', exact: true })).toBeVisible()
        await expect.poll(
...
test('CA-013 persists create, replace, and clear Parent Company mutations while retaining Organizations', async ({ page }) => {
    ...
    await expect(ownership).toContainText('No parent company')
    const emptyParentControl = ownership.locator('.brand-parent-company-control')
    const emptyParentHeight = await emptyParentControl.evaluate((element) => element.getBoundingClientRect().height)
    const assignHeight = await ownership.getByRole('button', { name: 'Assign', exact: true }).evaluate((element) => element.getBoundingClientRect().height)
    expect(emptyParentHeight).toBeLessThanOrEqual(44)
    expect(assignHeight).toBeLessThanOrEqual(34)
    await expect(page.getByText('Draft', { exact: true })).toBeVisible()

    await chooseParentCompanyAction(ownership, 'Create Organization')
    ...
    await ownership.getByRole('button', { name: 'Change', exact: true }).click()
    ...
    await organizationPicker.fill('Apple Operations International')
    await manageDialog.getByRole('option', { name: 'Apple Operations International — Organization #1301602', exact: true }).click()
    ...
    await page.reload()
    await expect(ownership).toContainText('Apple Operations International')
    await expect(page.getByText('Draft', { exact: true })).toBeVisible()

    await chooseParentCompanyAction(ownership, 'Clear Parent Company')
    ...
    await ownership.getByRole('button', { name: 'Assign', exact: true }).click()
    ...
    await expect(page.getByRole('option', { name: /^Created Parent 株式会社 — Organization #\d+$/ })).toBeVisible()
    await retainedPicker.fill('Apple Operations International')
    await expect(page.getByRole('option', { name: 'Apple Operations International — Organization #1301602', exact: true })).toBeVisible()

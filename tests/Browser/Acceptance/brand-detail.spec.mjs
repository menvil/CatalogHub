...
test('CA-012 derives complete and needs-attention quality states from persisted Brand data', async ({ page }) => {
    ...
    await expect(completeIssues).toContainText('No open Brand quality issues.')
    ...
    await germanIssue.getByRole('link', { name: 'Review translation', exact: true }).click()
...
test('CA-012 completes the explicit lifecycle workflow and archive cancellation', async ({ page }) => {
    ...
    const logoIssue = page.locator('[data-screen-region="quality-issues"] [data-quality-issue-code]').filter({ hasText: 'Logo is missing' })
    await logoIssue.getByRole('link', { name: 'Manage logo', exact: true }).click()
...
test('CA-012 has no page-level overflow at intermediate viewports', async ({ page }) => {
    const assertNoPageErrors = observePageErrors(page)
    ...
    for (const viewport of [
        ...
        await expect.poll(
            () => page.evaluate(() => document.documentElement.scrollWidth <= document.documentElement.clientWidth),
            { message: `CA-012 must not overflow at ${viewport.width}px.` },
        ).toBe(true)
    }
